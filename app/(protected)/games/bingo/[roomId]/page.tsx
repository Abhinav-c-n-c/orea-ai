...
export default function BingoPage() {
  ...
  return (
    <main className="flex-1 flex flex-col h-full overflow-y-auto bg-slate-50 dark:bg-slate-900 pb-12 relative">
      ...
      <div className="bg-white dark:bg-slate-800 border-b border-surface-100 dark:border-slate-700 p-3 md:p-4 flex items-center justify-between shadow-sm sticky top-0 z-10">
        <button onClick={() => router.push('/games')} className="flex items-center gap-1.5 text-xs md:text-sm text-surface-600 dark:text-slate-300 hover:text-primary-600 font-bold transition-colors">
          <FiArrowLeft /> <span className="hidden xs:inline">Leave Game</span>
        </button>
        <div className="flex items-center bg-slate-100 dark:bg-slate-700/50 rounded-[4px] border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
          <div className="px-3 md:px-5 py-1.5 md:py-2 text-xs md:text-base font-mono font-black text-slate-900 dark:text-white uppercase tracking-wider border-r border-slate-200 dark:border-slate-700">
            <span className="hidden sm:inline text-[10px] text-slate-400 dark:text-slate-500 mr-2 uppercase tracking-normal font-bold">Room:</span>
            {currentRoom.roomId.toUpperCase()}
          </div>
          <button 
            onClick={copyRoomId} 
            className="px-3 md:px-4 py-1.5 md:py-2 bg-primary-500 hover:bg-primary-600 text-white transition-all flex items-center gap-1.5 text-[9px] md:text-[10px] font-black uppercase tracking-widest active:scale-95 shadow-inner"
          >
            {copied ? 'Done!' : <><FiCopy className="w-3" /> Copy</>}
          </button>
        </div>
      </div>

      <div className="max-w-5xl mx-auto w-full mt-4 md:mt-8 px-2 md:px-4 grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 items-start">
        ...
        <div className="lg:col-span-7 flex justify-center items-start order-1 lg:order-2">
          <div className="bg-white dark:bg-slate-800 p-2.5 md:p-6 rounded shadow-[0_30px_100px_-20px_rgba(0,0,0,0.15)] border border-surface-100 dark:border-slate-700 w-full max-w-md">
            ...
            <div className="mt-6 md:mt-8 flex justify-between items-center px-1 md:px-2">
              {letters.map((l, i) => {
                ...
                return (
                  <button
                    ...
                    className={`w-8 h-8 md:w-12 md:h-12 rounded flex items-center justify-center text-sm md:text-xl font-black transition-all duration-500 active:scale-95 disabled:cursor-not-allowed
                      ${isLit 
