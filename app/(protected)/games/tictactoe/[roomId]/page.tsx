...
export default function TicTacToePage() {
  ...
  const currentTurnId =
    typeof currentRoom.currentTurn === 'string'
      ? currentRoom.currentTurn
      : currentRoom.currentTurn?._id;
    ...
    <main className="flex-1 flex flex-col h-full overflow-y-auto bg-slate-50 dark:bg-slate-900 pb-12">
      <div className="bg-white dark:bg-slate-800 border-b border-surface-100 dark:border-slate-700 p-4 flex items-center justify-between shadow-sm">
        <button
          ...
          className="flex items-center gap-2 text-surface-600 dark:text-slate-300 hover:text-primary-600 font-medium transition-colors"
        >
        ...
        <div className="flex items-center bg-slate-100 dark:bg-slate-700/50 rounded-[4px] border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
          <div className="px-5 py-2 text-sm md:text-base font-mono font-black text-slate-900 dark:text-white uppercase tracking-wider border-r border-slate-200 dark:border-slate-700">
            <span className="text-[10px] text-slate-400 dark:text-slate-500 mr-2 uppercase tracking-normal font-bold">
              Room:
            </span>
      ...
      <div className="max-w-2xl mx-auto w-full mt-8 px-4 flex flex-col items-center">
          ...
          {currentRoom.status === 'waiting' ? (
            ...
            <div className="inline-flex flex-col items-center gap-2 px-8 py-4 rounded bg-gradient-to-br from-emerald-400 to-teal-500 text-white shadow-xl shadow-teal-500/30 transform scale-110">
              <span className="text-3xl font-black">
                {currentRoom.winner ? `${currentRoom.winner.name} Wins!` : "It's a Draw!"}
              </span>
              <span className="text-sm font-medium opacity-90">Game Over</span>
            </div>
          ) : (
            <div
              className={`inline-flex items-center gap-2 px-8 py-3 rounded font-bold shadow-md transition-colors ${isMyTurn ? 'bg-primary-500 text-white shadow-primary-500/30' : 'bg-surface-200 dark:bg-slate-700 text-surface-600 dark:text-slate-300'}`}
            >
        ...
        <div className="flex items-center justify-between w-full max-w-sm mb-12 relative">
          <div
            className={`flex flex-col items-center gap-2 transition-transform ${isPlayer1 ? 'scale-110' : 'opacity-70'} ${currentTurnId === player1?._id ? 'animate-pulse-soft' : ''}`}
          >
            <div className="w-16 h-16 rounded bg-indigo-500 flex items-center justify-center text-white text-3xl font-black shadow-lg shadow-indigo-500/30">
              X
            </div>
            <div className="bg-white dark:bg-slate-800 px-3 py-1.5 rounded shadow-sm border border-surface-100 dark:border-slate-700 flex items-center gap-2">
              <div className="w-5 h-5 rounded bg-indigo-100 flex items-center justify-center text-indigo-700 text-[10px] font-bold">
                P1
              </div>
              <span className="text-xs font-bold dark:text-white truncate max-w-[80px]">
          ...
          <div
            className={`flex flex-col items-center gap-2 transition-transform ${isPlayer2 ? 'scale-110' : 'opacity-70'} ${currentTurnId === player2?._id ? 'animate-pulse-soft' : ''}`}
          >
            <div className="w-16 h-16 rounded bg-rose-500 flex items-center justify-center text-white text-3xl font-black shadow-lg shadow-rose-500/30">
              O
            </div>
            <div className="bg-white dark:bg-slate-800 px-3 py-1.5 rounded shadow-sm border border-surface-100 dark:border-slate-700 flex items-center gap-2">
              <span className="text-xs font-bold dark:text-white truncate max-w-[80px] text-right">
                {player2?.name || 'Waiting...'}
              </span>
              <div className="w-5 h-5 rounded bg-rose-100 flex items-center justify-center text-rose-700 text-[10px] font-bold">
                P2
              </div>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center justify-center rounded md:rounded text-5xl md:text-7xl font-black transition-all duration-300 transform 
                    ${
                      !cellVal
                        ? 'bg-surface-50 dark:bg-slate-700 hover:bg-surface-100 dark:hover:bg-slate-600 hover:scale-[1.02] active:scale-95'
                        : isX
                          ? 'bg-indigo-50/50 dark:bg-indigo-500/10 text-indigo-500'
                          : 'bg-rose-50/50 dark:bg-rose-500/10 text-rose-500'
                    }
