...
export default function NotesPage() {
  ...
  const handleStartEdit = () => {
    ...
    setMentionedUserIds(
      selectedNote.mentions?.map((m) => (typeof m === 'string' ? m : (m as IUser)._id)) || []
    );
    setLinkedTaskIds(
      selectedNote.linkedTasks?.map((t) => (typeof t === 'string' ? t : (t as ITask)._id)) || []
    );
    setIsEditing(true);
  };
  ...
  const insertToolText = (tool: string) => {
    ...
    switch (tool) {
      case 'bold':
        ...
        break;
      case 'italic':
        toolText = '_italic text_';
        break;
      case 'list':
        toolText = '\n- list item';
        break;
      case 'image':
        toolText = '![alt text](image_url)';
        break;
      case 'link':
        toolText = '[link text](url)';
        break;
      case 'table':
        toolText = '\n| Title | Header |\n|-------|--------|\n| Content | Data |';
        break;
  ...
  return (
    <main className="flex-1 flex flex-col h-full overflow-hidden bg-surface-50 dark:bg-slate-950 p-0 md:p-6 gap-0 md:gap-6">
      <div className="flex-1 flex min-h-0 gap-0 md:gap-6">
        {/* Left Sidebar - Notes List */}
        <motion.div
          initial={{ x: -20, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          className={`${selectedNote ? 'hidden md:flex' : 'flex'} w-full md:w-80 flex-col bg-white dark:bg-slate-900 rounded-none md:rounded-[4px] border-b md:border border-primary-100 dark:border-slate-800 shadow-xl overflow-hidden`}
        >
          <div className="p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-black text-surface-900 dark:text-white uppercase tracking-tighter">
                Library
              </h2>
              <button
              ...
              {notes.map((note) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <span className="text-[10px] font-bold text-surface-400 dark:text-slate-500 uppercase tracking-widest">
                  ...
                  <h4
                    className={`font-bold text-sm mb-1 line-clamp-1 transition-colors ${selectedNote?._id === note._id ? 'text-primary-700 dark:text-primary-400' : 'text-surface-900 dark:text-slate-200'}`}
                  >
                    {note.title}
                  </h4>
            ...
            {!isLoading && notes.length === 0 && (
              <div className="text-center py-12 px-6">
                ...
                <p className="text-xs text-surface-400 italic">
                  No notes found. Create your first masterpiece.
                </p>
              </div>
            )}
          </div>
        ...
        <motion.div
          ...
          className={`${selectedNote ? 'flex' : 'hidden md:flex'} flex-1 bg-white dark:bg-slate-900 rounded-none md:rounded-[4px] shadow-xl overflow-hidden flex flex-col relative`}
        >
          {selectedNote ? (
            <>
              {/* Toolbar Top */}
              <div className="px-3 md:px-8 py-3 md:py-4 border-b border-primary-50 dark:border-slate-800 flex items-center justify-between bg-surface-50/30 dark:bg-slate-900/50 backdrop-blur-md sticky top-0 z-20">
                <div className="flex items-center gap-2 md:gap-4">
                  ...
                  <div className="flex items-center gap-1.5 px-2 md:px-3 py-1 bg-white dark:bg-slate-800 rounded-full shadow-sm border border-primary-50 dark:border-slate-700">
                    <Clock className="w-3 md:w-3.5 h-3 md:h-3.5 text-primary-500" />
                    <span className="text-[8px] md:text-[10px] font-black text-surface-500 dark:text-slate-400 uppercase tracking-tighter">
                      <span className="hidden xs:inline">SAVED </span>{timeAgo(selectedNote.updatedAt).toUpperCase()}
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-1 md:gap-2">
                  {!isEditing ? (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-4 md:px-6 py-1.5 md:py-2 bg-primary-600 text-white rounded-[4px] text-[10px] md:text-sm font-black uppercase tracking-widest hover:bg-primary-700 transition-all shadow-lg shadow-primary-500/20"
                    >
                      <Edit3 className="w-3.5 md:w-4 h-3.5 md:h-4" /> <span className="hidden xs:inline">Edit</span>
                    </button>
                  ) : (
                    <div className="flex items-center gap-1 md:gap-2">
                      <button
                        onClick={() => setIsEditing(false)}
                        className="px-2 md:px-4 py-1.5 md:py-2 text-surface-500 text-[10px] md:text-sm font-black uppercase tracking-widest hover:text-surface-900 dark:hover:text-white transition-colors"
                      >
                        <span className="xs:hidden">Cancel</span><span className="hidden xs:inline">Cancel</span>
                      </button>
                      <button
                        onClick={handleSave}
                        className="flex items-center gap-1.5 px-4 md:px-6 py-1.5 md:py-2 bg-emerald-600 text-white rounded-[4px] text-[10px] md:text-sm font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-500/20"
                      >
                        <Save className="w-3.5 md:w-4 h-3.5 md:h-4" /> <span className="hidden xs:inline">Save</span>
                      </button>
                    </div>
                  )}
                  <div className="h-5 md:h-6 w-px bg-surface-100 dark:bg-slate-800 mx-1 md:mx-2" />
                  <button
                    onClick={async () => {
                    ...
                    className="p-1.5 md:p-2 hover:bg-red-50 dark:hover:bg-red-500/10 text-surface-400 hover:text-red-500 rounded-xl transition-all"
                  >
              ...
              <div className="flex-1 overflow-y-auto px-4 md:px-12 py-6 md:py-10 custom-scrollbar scroll-smooth bg-white dark:bg-slate-900">
                {isEditing ? (
                  <motion.div
                  ...
                  >
                    <input
                      ...
                      placeholder="Title"
                      className="text-xl md:text-2xl font-black text-surface-900 dark:text-white w-full border-none outline-none bg-transparent placeholder:text-surface-200 dark:placeholder:text-slate-800 tracking-tight uppercase"
                    />
                    ...
                    <div className="relative">
                      ...
                      <div className="flex items-center gap-1 p-1 bg-white dark:bg-slate-800 rounded-[4px] shadow-md border border-primary-100 dark:border-slate-700 mb-4 inline-flex">
                        <button
                          onClick={() => insertToolText('bold')}
                          className="p-2 hover:bg-surface-50 dark:hover:bg-slate-700 rounded-lg text-surface-500 transition-colors"
                          title="Bold"
                        >
                          <Bold className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => insertToolText('italic')}
                          className="p-2 hover:bg-surface-50 dark:hover:bg-slate-700 rounded-lg text-surface-500 transition-colors"
                          title="Italic"
                        >
                        ...
                        <div className="relative">
                          ...
                          <AnimatePresence>
                            {showTableSelector && (
                              ...
                              >
                                <div className="text-[10px] font-black uppercase tracking-widest text-surface-400 mb-2">
                                  Select Grid{' '}
                                  {hoveredGrid.r > 0 ? `${hoveredGrid.r}x${hoveredGrid.c}` : ''}
                                </div>
                                ...
                                <div className="flex flex-col gap-3 mt-4 pt-4 border-t border-primary-50 dark:border-slate-700">
                                  <div className="flex items-center gap-2">
                                    <div className="flex-1">
                                      <p className="text-[10px] font-bold text-surface-400 uppercase mb-1">
                                        Rows
                                      </p>
                                      <input
                                        type="number"
                                        value={inputRows}
                                        onChange={(e) =>
                                          setInputRows(parseInt(e.target.value) || 1)
                                        }
                                    ...
                                    <div className="flex-1">
                                      ...
                                      <input
                                        ...
                                        value={inputCols}
                                        onChange={(e) =>
                                          setInputCols(parseInt(e.target.value) || 1)
                                        }
                                        className="w-full px-2 py-1.5 bg-surface-50 dark:bg-slate-900 border border-primary-100 dark:border-slate-700 rounded-[4px] text-xs outline-none focus:ring-1 focus:ring-primary-500 dark:text-white"
                                      />
                                    </div>
                        ...
                        <div className="w-px h-4 bg-surface-100 mx-1" />
                        <button
                          className="p-2 hover:bg-surface-50 dark:hover:bg-slate-700 rounded-lg text-surface-500 flex items-center gap-1.5 px-3 transition-colors"
                          title="Link Task"
                        >
                          ...
                          >
                            <div className="p-4 bg-surface-50 dark:bg-slate-900 border-b border-primary-50 dark:border-slate-800">
                              <span className="text-[10px] font-black uppercase tracking-widest text-primary-600">
                                {showTagPopover === 'user' ? 'Mention User' : 'Link Task'}
                              </span>
                            </div>
                            <div className="max-h-60 overflow-y-auto p-2">
                              {showTagPopover === 'user'
                                ? users
                                    .filter((u) =>
                                      u.name.toLowerCase().includes(tagFilter.toLowerCase())
                                    )
                                    .map((u) => (
                                      <button
                                        key={u._id}
                                      ...
                                      >
                                        ...
                                        <div className="flex flex-col">
                                          <span className="text-xs font-bold text-surface-900 dark:text-white group-hover:text-primary-600">
                                            {u.name}
                                          </span>
                                ...
                                : tasks
                                    .filter((t) =>
                                      t.title.toLowerCase().includes(tagFilter.toLowerCase())
                                    )
                                    .map((t) => (
                                      <button
                                        key={t._id}
                                      ...
                                      >
                                        <div className="flex items-center gap-2 mb-1">
                                          ...
                                          <span className="text-xs font-bold text-surface-900 dark:text-white group-hover:text-emerald-600 truncate">
                                            {t.title}
                                          </span>
                                        </div>
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 sm:items-center border-y border-surface-50 dark:border-slate-800 py-4 md:py-6">
                      <div className="flex items-center gap-2 md:gap-3 sm:pr-6 sm:border-r border-surface-50 dark:border-slate-800">
                        <div className="w-6 md:w-8 h-6 md:h-8 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white font-bold text-[8px] md:text-[10px]">
                          {getInitials(selectedNote.author?.name || 'U')}
                        </div>
                        <div className="flex flex-col">
                          <span className="text-[8px] md:text-[10px] font-black text-surface-400 uppercase tracking-widest">
                            Author
                          </span>
                          <span className="text-[10px] md:text-xs font-bold text-surface-900 dark:text-white">
                      ...
                      <div className="flex items-center gap-2 md:gap-3 sm:pr-6 sm:border-r border-surface-50 dark:border-slate-800">
                        <Calendar className="w-4 md:w-5 h-4 md:h-5 text-primary-500" />
                        <div className="flex flex-col">
                          <span className="text-[8px] md:text-[10px] font-black text-surface-400 uppercase tracking-widest">
                            Updated
                          </span>
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap ml-auto">
                        {selectedNote.tags?.map((t) => (
                          <span
                            key={t}
                            className="px-2 md:px-3 py-0.5 md:py-1 bg-surface-50 dark:bg-slate-800 text-surface-500 dark:text-slate-400 rounded-full text-[8px] md:text-[10px] font-bold uppercase tracking-widest border border-primary-50 dark:border-slate-700"
                          >
                            #{t}
                          </span>
                        ))}
                    ...
                    {(selectedNote.mentions?.length > 0 ||
                      selectedNote.linkedTasks?.length > 0) && (
                      <div className="pt-12 space-y-6">
                        <h5 className="text-[10px] font-black text-surface-400 uppercase tracking-widest border-b border-surface-50 dark:border-slate-800 pb-2">
                          Connections
                        </h5>
                        <div className="grid md:grid-cols-2 gap-4">
                          {selectedNote.mentions?.map((m) => {
                            ...
                            return (
                              <div
                                key={userObj._id}
                                className="flex items-center gap-3 p-4 bg-primary-50/30 dark:bg-primary-900/10 rounded-2xl border border-primary-100 dark:border-primary-900/20"
                              >
                          ...
                          {selectedNote.linkedTasks?.map((t) => {
                            ...
                            return (
                              <div
                                key={taskObj._id}
                                className="flex items-center gap-3 p-4 bg-emerald-50/30 dark:bg-emerald-900/10 rounded-2xl border border-emerald-100 dark:border-emerald-900/20"
                              >
            ...
            <div className="h-full flex flex-col items-center justify-center p-12 text-center">
              ...
              <h3 className="text-2xl font-black text-surface-900 dark:text-white mb-2 uppercase tracking-tighter">
                Your Mind's Canvas
              </h3>
